...
| `@deepseek-ai/dsh-diagram` | `diagram`, `diagram_read` | `ctx.tools`, `ctx.fs`, `ctx.systemPrompt` | `tool/call`, `fs/write-intent for the diagram write`, `fs/observed after the write`, `tool/result`, `fs/observed (present) for a diagram_read` | - | Writes deterministic Excalidraw `.excalidraw` documents from a validated shape spec; the expanded elements ride only in the replayable `presentationMeta` projection (bounded by `maxMetaBytes`), never in model-facing content. The `diagram_read` companion tool returns a bounded structural summary (types, geometry, labels, connector points) of an existing document, truncating at 200 elements with an explicit total. |
...
Source: [`packages/drawing/diagram/src/index.ts`](../packages/drawing/diagram/src/index.ts)
