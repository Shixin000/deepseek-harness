...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'diagram.card.open',
    kind: 'chain',
    scope: 'session',
    summary: 'Selector-routed extension rendered in the diagram card\'s header row: the first occupant that accepts the owner renders.',
    doc: 'Selector-routed extension rendered in the diagram card\'s header row:\nthe first occupant that accepts the owner renders. The card owner\nsupplies the file path the produced call wrote; a canvas panel registers\nhere to offer "Open in editor" without the card depending on it.',
    registerOptions: [
      {
        name: 'select',
        requirement: 'required',
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'tool.call.toolview\' (client-ui-diagram), so it exists while that entry is mounted',
    occupants: [
      'client-ui-diagram-canvas OpenInEditorButton',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'diagram.card.open\', () => ctx.slots.register(\n      { name: \'diagram.card.open\', select: owner => null },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-diagram/src/client/contract/slots.ts:12',
  },
  {
    key: 'main',
    kind: 'keyed',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-diagram-canvas CanvasPanel id \'diagram-canvas\'',
  ...
  },
  {
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: ask_user_question, bash, cordis_define, cordis_run, cordis_stop, cordis_undefine, diagram, edit, glob, grep, present, read, read_image, skill, todo_write, web_fetch, web_search, write',
    hookContext: '',
    slotInject: '',
    ...
    occupants: [
      ...
      'client-ui-diagram DiagramCard key \'diagram\'',
